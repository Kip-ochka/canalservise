    ...
    _getAlbums = (userData) => {
        if (userData != null) {
            return axios.get('https://jsonplaceholder.typicode.com/albums').then(({data}) => {
                const needAlbums = []
                data.sort(album => album.albumId).forEach(album => {
    ...
    _getPhotos = (albums) => {
        if (albums != null) {
            axios.get(`https://jsonplaceholder.typicode.com/photos`).then(({data}) => {
                ...
                data.sort(x => x.photoId).forEach((photo) => {
                    if (albumFirstPhoto.findIndex(x => x.albumId == photo.albumId) == -1) {
    ...
    _getPost = (userData) => {
        if (userData != null) {
            axios.get(`https://jsonplaceholder.typicode.com/posts`).then(({data}) => {
                const userFirstPost = [];
                data.sort(x => x.postId).forEach((post) => {
